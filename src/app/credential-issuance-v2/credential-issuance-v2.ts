...
export class CredentialIssuanceV2 implements OnInit {
    ...
    constructor(private apiService: ApiService, private credentialService: CredentialService) {
    }
    ...
    onResolve(input: string): void {
        ...
        this.apiService.resolveOpenIdMetadataFromDeeplink(decodedDeeplink?.credential_issuer)
            .pipe(
                switchMap((metadata) => {
                    if (!!metadata) {
                        this.metadata.set(metadata);
                        this.extractCredentialConfigurationsSupported(decodedDeeplink, metadata);
                        return this.apiService.resolveOpenIdConfigMetadataFromDeeplink(decodedDeeplink?.credential_issuer)
                    } else {
                        return of(null);
                    }
                }),
                switchMap((openIdConfig: any) => {
                    ...
                    return this.apiService.getAccessToken(decodedDeeplink?.grants?.['urn:ietf:params:oauth:grant-type:pre-authorized_code']?.['pre-authorized_code'], openIdConfig?.token_endpoint);
                }),
                switchMap((accessToken: any) => {
                    this.tokenResponse.set(accessToken);

                    return from(this.apiService.getNonce(this.metadata()?.["nonce_endpoint"]));
                }), switchMap((nonce: any) => {
                    this.nonceResponse.set(nonce?.c_nonce);

                    return from(this.getCredentialRequestV2(nonce));
                }), switchMap((request: any) => {
                    return this.apiService.getCredential(this.metadata()?.["credential_endpoint"], this.tokenResponse()?.access_token, request);
    ...
    checkIfKeyPresent(): boolean {
        ...
        const verificationMethods: any[] = this.registryEntry()[3]?.value?.verificationMethod;
    ...
    private extractCredentialConfigurationsSupported(decodedDeeplink: any, metadata: any): any {
        this.credentialConfig.set(metadata?.credential_configurations_supported?.[decodedDeeplink?.credential_configuration_ids?.[0]]);
    }

    private async getCredentialRequestV2(nonce: string): Promise<any> {

        const proofSigningAlgValuesSupported = this.credentialConfig()?.proof_types_supported?.jwt?.proof_signing_alg_values_supported[0];

        const jwt = await this.credentialService.createHolderBinding(this.metadata()?.credential_issuer, nonce, proofSigningAlgValuesSupported, this.privateKey, this.jwk);

        return {
            'credential_identifier': this.deeplink()?.credential_configuration_ids?.[0],
            'proofs': {
                'jwts': [
                    jwt
                ]

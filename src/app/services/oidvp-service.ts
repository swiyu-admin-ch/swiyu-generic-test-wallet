...
export class OIDVPService {
  ...
  fetchRequestObject(
    requestObjectUrl: string,
  ): Observable<RequestObject | string> {
    if (!requestObjectUrl) {
  ...
  private preparePayload(
    ...
    return of({
      body: new HttpParams()
        ...
        .set("state", requestObject.state ? requestObject.state : ""),
      headers,
    });
  }
  ...
  private async encryptVerifierPayload(
    ...
    const encryptionEnc =
      requestObject.client_metadata
        ?.encrypted_response_enc_values_supported?.[0];
    if (!encryptionEnc) {
      throw new Error(
        "No encryption algorithm specified in encrypted_response_enc_values_supported",
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Invalid vp_token JSON format: ${(error as Error).message}`,
      );
    }
    ...
    const payload = new TextEncoder().encode(
      JSON.stringify({
        vp_token: vpToken,
        state: requestObject.state ? requestObject.state : null,
      }),
    );
